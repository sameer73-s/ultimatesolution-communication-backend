...
المحول الحالي يحتفظ بحالة الجلسات والتسجيلات العابرة داخل المثيل بهدف دعم تسلسل جلسة التطوير والاختبارات. عند اعتماد تشغيل متعدد المثيلات أو Jitsi خارجي كامل، تستبدل آلية الحالة داخل Infrastructure فقط؛ تبقى واجهة `IMeetingMediaService` وجميع Use Cases وواجهات API دون تعديل.
